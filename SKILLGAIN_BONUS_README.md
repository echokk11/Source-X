...
.show SKILLGAIN_BONUS_HOURS
.show SKILLGAIN_BONUS_MULTIPLIER
```

### 方法2：重新登录
最简单的方法是重新登录，登录时会自动显示剩余时间。

### 方法3：手动重置（如果出现问题）
如果显示的时间不正确，可以手动重置加成：
```
.xset SKILLGAIN_BONUS_STARTTIME 0
.xset SKILLGAIN_BONUS_DAY 0
```
重置后，下次技能增长时会自动激活新的加成。

### 注意事项
由于脚本中的 `<SERV.GAMETIME>` 和 C++ 代码中使用的时间基准不同，**不建议在脚本中手动计算剩余时间**。如果需要查看剩余时间，请重新登录查看登录消息。
